	...
	<tr>
		<td>
			<div class="ldauc-values">
				...
				<form name="uploadfile"  class="ldauc-form" id="uploadfile_form" method="POST" enctype="multipart/form-data" action="" accept-charset="utf-8" >
					<input type="file" name="uploadfiles[]" id="uploadfiles" size="35" class="uploadfiles" />
					<input type="hidden" value="<?php echo $post->ID ?>" name="post"/>
					<input type="hidden" name="uploadfile" value="<?php echo wp_create_nonce( 'uploadfile_'. get_current_user_id() .'_'. $post->ID ) ?>"  />
					<input class="button-primary" type="submit" id="uploadfile_btn" value="<?php echo  __( 'Upload', 'ldauc' )  ?>"  />
				</form>
			</div>
		</td>
	</tr>
